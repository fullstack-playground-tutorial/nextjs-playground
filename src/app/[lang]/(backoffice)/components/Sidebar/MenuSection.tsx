...
export const MenuSectionInternal = (props: MenuSectionInternalProps) => {
  ...
  return (
    <section
      className={`flex flex-col w-full z-1 mb-1 transition-all ${
        !topbar
          ? menuExpand
            ? "px-2"
            : "items-center"
    ...
    >
      ...
      >
        {url ? (
          <Link
            href={url}
            className={`flex flex-row gap-3 cursor-pointer w-full p-2 px-3 ${
              topbar ? "items-center" : "items-center"
            }`}
          >
            <>
              {_Icon ? (
          ...
          >
            ...
            <label
              className={`cursor-pointer truncate text-md font-medium transition-colors duration-200 ${
                topbar ? "" : menuExpand ? "" : "hidden"
              } text-gray-600 dark:text-secondary group-hover:text-gray-900 dark:group-hover:text-primary`}
            >
              {title}
        ...
        <div
          className={`flex flex-row gap-1 pr-2 ${
            menuExpand ? "" : "hidden"
          } items-center`}
        >
          {!topbar && (
            ...
            >
              {propPinned ? (
                ...
                <KeepIcon
                  className={`rotate-45 fill-gray-400 dark:fill-secondary hover:fill-primary size-3.5`}
                />
              )}
            </button>
          )}
      ...
      {clonedChildren && (
        <div
          className={`${
            topbar
              ? `section-list bg-white dark:bg-surface-1 border dark:border-border rounded-md shadow-lg py-1 min-w-[200px] z-50 ${
                  path.length <= 1
