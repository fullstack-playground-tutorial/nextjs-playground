...
export default function QuizForm({
  mode,
  fetchedQuizPromise: quizPromise,
}: Props) {
    ...
    <form className="flex flex-col gap-6 w-full bg-white dark:bg-surface-1 p-8 rounded-xl border dark:border-border shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col gap-2">
          ...
          />
          {errors.title && (
            <span className="text-alert-0 text-xs">{errors.title}</span>
          )}
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-xs font-bold text-secondary uppercase tracking-wider">
      ...
      <div className="flex flex-col gap-2">
        ...
        {errors.questions && (
          <span className="text-alert-0 text-center text-xs mt-2 font-bold">
            {errors.questions}
          </span>
        )}
      </div>
            ...
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    ...
                    {errors[`questions.${index}.point`] && (
                      <span className="text-alert-0 text-[10px]">
                        {errors[`questions.${index}.point`]}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {errors[`questions.${index}.content`] && (
                  <span className="text-alert-0 text-xs">
                    {errors[`questions.${index}.content`]}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {item.answers?.map((c, cIndex) => (
                  <div key={cIndex} className="flex flex-col gap-1">
                    <div className="flex gap-3 items-center">
                      <input
                        type={
                          item.type === "single_choice" ? "radio" : "checkbox"
                        }
                        name={`question-${index}`}
                    ...
                    {errors[`questions.${index}.answers.${cIndex}.content`] && (
                      <span className="text-alert-0 text-[10px] ml-8">
                        {errors[`questions.${index}.answers.${cIndex}.content`]}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {errors[`questions.${index}.answers`] && (
                    <span className="text-alert-0 text-xs px-2">
                      {errors[`questions.${index}.answers`]}
                    </span>
                  )}
                  {errors[`questions.${index}.correctAnswer`] && (
                    <span className="text-alert-0 text-xs px-2 font-bold">
                      {errors[`questions.${index}.correctAnswer`]}
                    </span>
                  )}
                </div>
            ...
            >
              <div className="flex justify-between items-center">
                ...
                {Object.keys(errors).some((k) =>
                  k.startsWith(`questions.${index}`),
                ) && (
                  <span className="text-alert-0 text-[10px] font-bold">
                    HAS ERRORS
                  </span>
                )}
              </div>
              <p className="font-semibold dark:text-primary">{item.content}</p>
