...
function InterestsManagement({ hasPermission, limit, currentPage, data }: Props) {
        ...
        <>
            <div className="h-screen flex flex-col items-start max-w-300 mx-auto p-4 mt-2">
                <div className="flex flex-row mb-6 gap-4 w-full">
                    <h1 className="font-semibold dark:text-accent-0 text-4xl">Interests</h1>
                </div>
                ...
                <div className="grid mt-4 md:mt-6 lg:mt-8 xl:mt-10 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mx-auto">
                    {pending
                        ? Array.from({ length: limit }).map((_, i) => {
                            return (
                                <div key={i} className="xl:w-70 xl:h-42 max-h-42 h-auto md:w-65 rounded-xl overflow-hidden">
                                    <SkeletonWrapper>
                                        <SkeletonElement
                                            width={"100%"}
                                            height={"100%"}
                ...
                {!pending && list.length === 0 && <div className="flex flex-col justify-center items-center w-full h-full">
                    <Image src={noData} alt="no-data" width={240} />
                    <p className="text-center text-base font-serif font-normal mt-4 text-primary">No interests found</p>
                </div>}
